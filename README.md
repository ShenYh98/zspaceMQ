# zspaceMQ
## 引言
这是一款用C++实现的轻量级的消息队列，原本应用在嵌入式系统上，因此这款消息队列中间件尽可能设计简单易用移植性强。至于功能性上的不足，欢迎留言评论补充，日后持续改进。

## 功能
* 线程间消息队列广播通信
* 线程间服务队列点对点通信
* 进程间消息队列广播通信（利用xpack序列化数据）
* 进程间服务队列点对点通信（利用xpack序列化数据）
...
## 编译环境配置
执行根目录下configure生成编译脚本和编译配置
```bash
./configure
```
直接执行脚本会在config路径下config.properties生成默认配置
```bash
PREFIX=/opt/zspaceMQ
CXX=g++
HOST=linux
